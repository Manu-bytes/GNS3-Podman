...
""".format(adapter=adapter, hostname=self._name))
    ...
    def _apply_rootless_ip_config(self, adapter_number):
        ...
        to the rootless container via podman exec.
        """
        ...
        log.info(f"Applying Rootless IP config for {interface}...")

        address = None
        netmask = None
        gateway = None

        # We read the configuration file generated by GNS3.
        try:
            ...
            for line in lines:
                line = line.strip()
                # We detect the block of our interface
                if line.startswith(f"iface {interface} inet static"):
                    in_iface_block = True
                    continue
